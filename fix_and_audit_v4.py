PATH_TO_PATIENT =
ROOT = #path to new_splits/
EICU_OUTPUT = #output directory from data_extraction_root
PATH_TO_EPISODE_MAPPER =
EICU_DATA_ROOT = 
SUFFIX = ""
IGNORE_UNDER = 15
#################
...
for filename in [f"inconsistent{p}.txt", f"listfile_err{p}.txt", f"audit_err{p}.txt", f"truncate{p}.txt"]:
...
for task in ["phenotyping_split/", "in-hospital-mortality_split/" ,"length-of-stay_split/", "decompensation_split/"]:
    ...
    for REGION in [ "south", "midwest", "west", "northeast"]:
        ...
        all_patients[task] |= uq_tr
        all_patients[task] |= uq_ts
        all_patients[task] |= uq_val
